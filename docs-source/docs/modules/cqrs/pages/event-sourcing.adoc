...
Aligning with the command handler above, the entity's event handler reacts on events and updated the state. Notice that there are no decisions on events, they are applied without any checks.
